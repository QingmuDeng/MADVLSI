...
.control
  ...
  while run <= runs
    ...
    while code < 128
      ...
      save all
      op
      wrdata ~/Documents/dactut/mcdactut\{$&run\}.txt v(b0) v(b1) v(b2) v(b3) v(b4) v(b5) v(b6) i(Viout) v(Vout)
      if code eq 0
        set appendwrite
...
value=10k}
C {/home/madvlsi/Documents/dactut/gaussresdef.sym} 910 -190 0 0 {name=GAUSSRESDEF only_toplevel=false value=".param MC_SWITCH=1.0
